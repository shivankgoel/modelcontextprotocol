---
title: "SEP-2007: Payment Support for MCP Servers"
sidebarTitle: "SEP-2007: Payment Support for MCP Servers"
description: "Payment Support for MCP Servers"
---
...
| ------------- | ------------------------------------------------------------------------------- |
| **SEP**       | 2007                                                                            |
| **Title**     | Payment Support for MCP Servers                                                 |
| **Status**    | Draft                                                                           |
| **Type**      | Standards Track                                                                 |
| **Created**   | 2025-12-23                                                                      |
| **Author(s)** | shivankgoel                                                                     |
| **Sponsor**   | LucaButBoring                                                                   |
| **PR**        | [#2007](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2007) |
...
```typescript
async onPaymentRequested(toolName, paymentOptions) {
  // Check if user has sufficient balance for any protocol
  const affordableOptions = paymentOptions.filter(opt =>
    canAfford(opt.protocol, opt.paymentRequired)
  );
  ...
  // Prefer crypto over fiat if available
  const cryptoOption = affordableOptions.find(opt => opt.protocol === "x402");
  if (cryptoOption) {
    return { type: "select", protocol: "x402" };
  }

  // Otherwise use first available option
  return { type: "select", protocol: affordableOptions[0].protocol };
}
```
...
```typescript
async onPaymentError(toolName, protocol, error) {
  console.error(`Payment failed for ${toolName} using ${protocol}:`, error);

  // Log to analytics
  trackPaymentFailure(toolName, protocol, error);
...
```typescript
async onPaymentSuccess(toolName, protocol, paymentResult) {
  // Update local spend tracking
  await updateSpendLog({
    tool: toolName,
    protocol: protocol,
    timestamp: Date.now(),
