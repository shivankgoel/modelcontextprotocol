...
- **Payment**: Payment handling for premium features using X402 protocol
...
MCP provides a [Payment](/specification/draft/basic/payment) framework for premium features and usage-based billing using various payment protocols, with X402 Protocol v2 as the recommended first protocol.

The payment specification enables servers to request payment for specific tools or resources while maintaining security through protocol-specific facilitators. Payment handling works across all MCP transports including HTTP, STDIO, and WebSocket.
